...
void process_packet(int c_id, char* packet)
{
    ...
    case CS_LOGIN: {
        ...
        {
            lock_guard<mutex> ll{ it->second->_s_lock };
            it->second->x = rand() % W_WIDTH;
            it->second->y = rand() % W_HEIGHT;
    ...
    case CS_MOVE: {
        ...
        it->second->_vl.lock();
        unordered_set<int> old_vlist = it->second->_view_list;
        ...
        it->second->send_move_packet(c_id);
        ...
        for (auto& pl : near_list) {
            ...
            if (old_vlist.count(pl) == 0)
                it->second->send_add_player_packet(pl);
        }

        for (auto& pl : old_vlist)
            if (0 == near_list.count(pl)) {
                it->second->send_remove_player_packet(pl);
