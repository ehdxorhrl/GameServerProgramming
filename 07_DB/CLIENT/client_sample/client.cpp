...
void ProcessPacket(char* ptr)
{
	...
	case SC_LOGIN_INFO:
	{
		...
		std::string uid_str = std::to_string(my_id);
		avatar.set_name(uid_str.c_str());
	...
	case SC_ADD_OBJECT:
	{
		...
		if (id == g_myid) {
			...
			std::string user_id_str = std::to_string(my_packet->uid);
			players[id].set_name(user_id_str.c_str());
			players[id].show();
		}
		else {
...
int main()
{
	...
	cout << "Input server IP (default: 127.0.0.1): ";
	cin.getline(server_ip, 32);
	if (strlen(server_ip) == 0) strcpy_s(server_ip, "127.0.0.1");

	// 사용자 ID 입력 받기
	cout << "Input your user ID (integer): ";
	cin >> my_id;
	cin.ignore(); // 남아있는 '\n' 제거 (getline 다음 cin 대비)
